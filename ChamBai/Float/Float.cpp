...
bool cmp(ld a, ld b){
    ...
    if (abs(a-b)/max((ld)1, abs(a))<eps) return 1;
    return 0;
}
...
bool checker(string s1, string s2){
    ...
    ld s3,s4;
    vector<ld> V1,V2;
    while (out1>>s3) V1.pb(s3);
    while (out2>>s3) V2.pb(s3);
    if (V1.size()>V2.size()) {REPORT<<"Not enough numbers/ words!"<<endl<<endl;return 0;}
    if (V1.size()<V2.size()) {REPORT<<"Too much numbers/ words!"<<endl<<endl; return 0;}
    int i=0;
    while (i<V1.size()){
        s3=V1[i];
        s4=V2[i];
        if (!cmp(s3,s4)) {
            REPORT<<fixed<<setprecision(8)<<"WA on position "<<i+1<<" : Expected "<<s3<<", found "<<s4<<", diff: "<< abs(s3-s4)/max((ld)1, abs(s4))<<endl<<endl;
            return 0;
        }
        i++;
    }
